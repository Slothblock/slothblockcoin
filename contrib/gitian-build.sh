...
SIGNER=abcd
VERSION=1.14.7
url=https://github.com/connorl953/slothblockcoin
proc=2
mem=2000
...
Standalone script to perform the gitian build of SlothBlockCoin Core. Perform
...
-u|--url repo       Specify the URL of the repository. Default is https://github.com/slothblockcoin/slothblockcoin
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/slothblockcoin-binaries/${VERSION}/ \;
}
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/slothblockcoin-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean slothblockcoin git directory because of old caching
    if [ -d inputs/slothblockcoin/ ]; then
        echo "Cleaning SlothBlockCoin directory..."
        rm -rf inputs/slothblockcoin/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit slothblockcoin="$COMMIT" --url https://github.com/connorl953/slothblockcoin.git ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
