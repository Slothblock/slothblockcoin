...
Below are some notes on how to build SlothBlockCoin Core for Windows.
...
To build SlothBlockCoin from source code, you'll need the source code. Either check it out via `git` or download
a zip file. (Look at the green "<> Code" button on [the SlothBlockCoin GitHub repository](https://github.com/slothblockcoin/slothblockcoin/)).
...
Note that for WSL v1 the SlothBlockCoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/slothblockcoin, AND not under, for example, /mnt/d/slothblockcoin.
...
Ensure that this toolchain can build for `posix` (else you may have compilation errors for SlothBlockCoin's dependencies):
...
way. This will install to `c:\workspace\slothblockcoin`, for example:

    make install DESTDIR=/mnt/c/workspace/slothblockcoin
