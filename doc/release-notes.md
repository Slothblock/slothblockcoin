SlothBlockCoin Core version 1.14.7 is now available from:

  <https://github.com/slothblockcoin/slothblockcoin/releases/tag/v1.14.7/>

This is a new minor version release, including enchancements to several RPC
methods and important security updates for SlothBlockCoin-Qt. SlothBlockCoin-Qt users on
...
To receive notifications about updates, subscribe to the release mailing list:
  ...
  <https://sourceforge.net/projects/slothblockcoin/lists/slothblockcoin-releases>


Compatibility
==============

SlothBlockCoin Core is extensively tested on Ubuntu Server LTS, macOS and Windows.
...
This release contains fixes for SlothBlockCoin Qt across all platforms that increase
security for end users.

### Disable BIP-70 payment server by default

To mitigate future potential risk inside SlothBlockCoin Qt, support for BIP-70 payment
...
inside SlothBlockCoin Qt. By default, incoming BIP-70 payment requests and files or
...
slothblockcoin.conf or as an argument to SlothBlockCoin-Qt if absolutely needed. Please use
extreme caution when exercising this option.

This is step 1 in full deprecation of BIP-70, BIP-71 and BIP-72 inside SlothBlockCoin
...
* Updated the `rpcuser.py` script to improve the password and salt generation
  ...
  current authentication in `slothblockcoin.conf` with new credentials generated with
...
Fee estimation was explicitly not ported or supported on SlothBlockCoin Core since
...
to support SlothBlockCoin parametrization.
...
```
slothblockcoin-cli help getblock
```
...
```
slothblockcoin-cli help getblockstats
```
...
* Fixed the example `slothblockcoind.service` file (#3066)
* Fixed a bug where misnamed signal handlers were causing excessive warnings
  in SlothBlockCoin Qt logs (#3063)
...
* Updated the python `ltc-scrypt` module to a maintained fork (#3080), which
  can be found at https://github.com/slothblockcoin/ltc-scrypt
