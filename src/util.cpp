...
const char * const BITCOIN_CONF_FILENAME = "slothblockcoin.conf";
const char * const BITCOIN_PID_FILENAME = "slothblockcoind.pid";
...
#else
    const char* pszModule = "slothblockcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SlothBlockCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SlothBlockCoin";
#else
    // Unix
    return pathRet / ".slothblockcoin";
#endif
#endif
}
