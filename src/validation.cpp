...
#if defined(NDEBUG)
# error "SlothBlockCoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "SlothBlockCoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetSlothBlockCoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool IsInitialBlockDownload()
{
    ...
    LOCK(cs_main);
    if(chainActive.Height() < 500000){ // Chain may go down in the beginning. Prevent chain from locking itself up.
        return false;
    }
    if (latchToFalse.load(std::memory_order_relaxed))
        return false;
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // SlothBlockCoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("slothblockcoin-scriptch");
    ...
    if (block.GetHash() != Params().GetConsensus(0).hashGenesisBlock)
{
    ...
    // SlothBlockCoin: BIP30 has been active since inception
    ...
    // SlothBlockCoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetSlothBlockCoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // SlothBlockCoin: Disallow AuxPow blocks before it is activated.
    ...
    // SlothBlockCoin: Version 2 enforcement was never used
    if(nHeight > 1){
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // SlothBlockCoin: We probably want to disable this
    int nLockTimeFlags = 0;
...
//Comment this out to improve compatibility with existing mining software
