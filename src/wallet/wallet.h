...
/* 1.14.5: set the wallet's discard threshold to 1 SLBC because that's what 97%
 ...
 */
/* 1.14.6: set the wallet's discard threshold to 0.01 SLBC. Very network
...
/*
 * SlothBlockCoin: Scaled to 1/10th of the recommended transaction fee to make RBF
...
/*
 * SlothBlockCoin: Creating change outputs at exactly the dustlimit is counter-
...
public:
    ...
    /**
     * SlothBlockCoin: Get a fee targetting a specific transaction speed.
     */
    CAmount GetSlothBlockCoinPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed);
    /**
     * SlothBlockCoin: Get a fee targetting a specific transaction speed.
     */
    static CAmount GetSlothBlockCoinPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed, CAmount targetFee);
    /**
    ...
    /**
     * Return the minimum required fee taking into account the
     * floating relay fee and user set minimum transaction fee, but not the SlothBlockCoin dust fee.
     */
